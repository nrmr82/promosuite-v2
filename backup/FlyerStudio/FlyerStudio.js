...
const FlyerStudio = forwardRef(({ 
  ...
  const optimizeLayoutLocally = useCallback(() => {
    ...
    if (!store || !store.pages || store.pages.length === 0) {
      console.warn('No pages available for optimization');
      alert('Please add some content to your flyer first!');
      return;
    }
    ...
    if (!page || !page.children || page.children.length === 0) {
      console.warn('No elements on page to optimize');
      alert('Please add some elements to your flyer first!');
      return;
    }
    ...
    elements.forEach((element, index) => {
      ...
      if (index > 0) {
        ...
        const prevBottomY = (prevElement.y || 0) + (prevElement.height || 50);
    ...
    alert('Layout optimized! Elements have been aligned and spaced.');
  }, [store]);
